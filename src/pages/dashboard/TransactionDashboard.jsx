...
const TransactionDashboard = () => {
  ...
  const [sortOrder, setSortOrder] = useState('desc'); // NEW: Working sort feature
  ...
  // 1. CONNECTED: Auth Guard & Master Fetch
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        if (data && data.length > 0) {
          // REALITY: Map exact data without mock fallbacks
          const mappedData = data.map((tx) => {
            ...
            const assignedColor = colors[charCode % colors.length];
            ...
            return {
              ...
              description: tx.service_provided || tx.description || 'No description provided',
              ...
              ...tx // Keep raw data for passing to receipt
            };
          });
          
          setTransactions(mappedData);

          // REALITY: Generate real notifications from pending/disputed items
          const alerts = mappedData
            .filter(t => t.status === 'Disputed' || t.status === 'Pending')
            .slice(0, 4);
          setNotifications(alerts);

        } else {
          setTransactions([]); // ZERO MOCK DATA
        }
      } catch (err) {
  ...
  const handleExportCSV = () => {
    ...
    const csvContent = [
      headers.join(','),
      ...transactions.map(t => `"${t.id}","${t.clientName}","${t.description}","${t.rawAmount}","${t.date}","${t.status}"`)
    ].join('\n');
  ...
  // REALITY: Comprehensive Filtering & Sorting Logic
  const filteredTransactions = transactions
    ...
    .sort((a, b) => sortOrder === 'desc' ? b.rawDate - a.rawDate : a.rawDate - b.rawDate);
  ...
  const verificationRate = transactions.length > 0 
    ? Math.round((transactions.filter(t => t.status === 'Verified').length / transactions.length) * 100) 
    : 0;
      ...
      <main className="flex-1 lg:ml-64 min-h-screen flex flex-col w-full">
        ...
        <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-4 md:px-8 sticky top-0 z-20">
          ...
          <div className="hidden sm:flex items-center gap-4 pl-5 border-l border-slate-200">
            ...
            {/* REALITY: Working Notification Bell */}
                  ...
                  >
                    ...
                    <div className="space-y-4 max-h-[300px] overflow-y-auto">
                      {notifications.length === 0 ? (
                        <p className="text-xs text-slate-500 text-center py-2">You're all caught up!</p>
                      ) : (
                        notifications.map((n, i) => (
                          <div key={i} className={`flex gap-3 ${i > 0 ? 'border-t border-slate-50 pt-3' : ''}`}>
                            <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${n.status === 'Disputed' ? 'bg-rose-50 text-rose-600' : 'bg-amber-50 text-amber-600'}`}>
                              {n.status === 'Disputed' ? <AlertCircle size={14}/> : <ShieldCheck size={14}/>}
                            </div>
                            <div>
                              <p className="text-xs font-bold text-slate-800">Transaction {n.status}</p>
                              <p className="text-[11px] text-slate-500 mt-0.5 leading-snug">Action needed for <span className="font-bold">{n.clientName}</span>.</p>
                            </div>
                          </div>
                        ))
                      )}
        ...
        <div className="p-4 md:p-8">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
            <div>
              <h1 className="text-2xl md:text-3xl font-black text-slate-900 tracking-tight">Ledger Operations</h1>
          ...
          {/* Bottom Stats Grid (Moved above table for better UX flow) */}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            <SummaryCard 
              label="Verified Total" 
              value={`₦${(verifiedTotal / 1000000).toFixed(2)}M`} 
              sub={<span className="text-emerald-500 font-bold flex items-center gap-1 mt-1"><TrendingUp size={12} /> Approved volume</span>} 
            />
            <SummaryCard 
              label="Pending Payouts" 
              value={`₦${(pendingTotal / 1000000).toFixed(2)}M`} 
              sub={<span className="text-amber-500 font-bold flex items-center gap-1 mt-1">🕓 {pendingCount} active request(s)</span>} 
            />
            <div className="bg-white p-6 md:p-8 rounded-[24px] border border-slate-200 shadow-sm transition-all hover:shadow-md">
              <p className="text-[10px] font-black text-slate-400 uppercase mb-2 tracking-widest">Verification Rate</p>
            ...
            <div className="bg-white p-6 md:p-8 rounded-[24px] border border-slate-200 shadow-sm transition-all hover:shadow-md">
              <p className="text-[10px] font-black text-slate-400 uppercase mb-2 tracking-widest">Disputed Cases</p>
              ...
              <p className={`text-[11px] font-bold flex items-center gap-1.5 mt-3 ${disputedCount > 0 ? 'text-rose-500 bg-rose-50 px-2 py-1 rounded-md w-max' : 'text-emerald-500 bg-emerald-50 px-2 py-1 rounded-md w-max'}`}>
                {disputedCount > 0 ? <><AlertCircle size={12}/> Action required</> : <><ShieldCheck size={12}/> All clear</>}
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
            ...
            <button 
              ...
              className="flex items-center gap-2 px-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm font-bold text-slate-600 hover:bg-slate-50 transition-colors w-full md:w-auto justify-center md:justify-start shadow-sm"
            >
              <ArrowUpDown size={16} /> Sort: {sortOrder === 'desc' ? 'Newest First' : 'Oldest First'}
            </button>
          </div>

          {/* Table Card */}
          <div className="bg-white border border-slate-200 rounded-[24px] md:rounded-[32px] shadow-sm overflow-hidden mb-8">
            <div className="overflow-x-auto">
              <table className="w-full text-left min-w-[900px]">
                <thead className="bg-[#f8fafc] border-b border-slate-100">
                  <tr className="text-[10px] md:text-[11px] uppercase font-black text-slate-400 tracking-widest">
                  ...
                  {isLoading ? (
                    ...
                    <tr>
                      <td colSpan="6" className="px-6 py-20 text-center">
                        ...
                        <p className="text-lg font-black text-slate-800 mb-1">No records found</p>
                    ...
                    currentItems.map((tx) => (
                      <tr key={tx.id} className="hover:bg-slate-50/80 transition-colors group cursor-pointer" onClick={() => navigate('/transaction-receipt', { state: { transactionData: tx } })}>
                        <td className="px-6 md:px-8 py-5">
                          <div className="flex items-center gap-3">
                            <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-white text-xs font-black shadow-sm ${tx.initialColor}`}>
                              {tx.initials}
                            </div>
                        ...
                        <td className="px-6 py-5">
                          <span className={`px-3 py-1.5 rounded-lg border text-[9px] md:text-[10px] font-black uppercase tracking-widest shadow-sm ${getStatusStyles(tx.status)}`}>
                            {tx.status}
                          </span>
                        </td>
                        <td className="px-6 md:px-8 py-5 text-right">
                          <button 
                            className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white border border-transparent rounded-lg text-[#1e40af] font-bold text-[11px] group-hover:bg-blue-50 group-hover:border-blue-200 transition-all shadow-sm active:scale-95"
                          >
            ...
            {filteredTransactions.length > 0 && (
              <div className="px-6 py-5 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-4 bg-[#f8fafc]">
                <p className="text-[11px] md:text-[12px] text-slate-400 font-bold uppercase tracking-widest text-center sm:text-left">
...
const SummaryCard = ({ label, value, sub }) => (
  <div className="bg-white p-6 md:p-8 rounded-[24px] border border-slate-200 shadow-sm flex flex-col justify-between transition-all hover:shadow-md">
    <div>
