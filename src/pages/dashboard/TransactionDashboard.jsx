  ...
  useEffect(() => {
      const fetchDashboardData = async () => {
        try {
          ...
          if (user) {
            // 1. FETCH PROFILE INFO (Corrected to user_id)
            const { data: profile } = await supabase
              .from('profiles')
              .select('business_name, business_type')
              .eq('user_id', user.id) // Fixed column name
              .maybeSingle();
            ...
            const { data, error } = await supabase
              .from('transactions')
              .select('*')
              .eq('user_id', user.id) // Ensure this matches your transaction table
            ...
            if (data && data.length > 0) {
              const mappedData = data.map((tx, index) => ({
                // Using index as fallback key to stop the 'unique key' error
                ...
                date: new Date(tx.created_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' }),
      ...
      fetchDashboardData();
    }, [navigate]); // Consistency in dependency
