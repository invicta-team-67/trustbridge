...
const TransactionSuccess = () => {
  ...
  // NEW: Store dynamic user data for the header
  const [userData, setUserData] = useState({ name: 'Loading...', role: 'Compliance Officer', avatarName: 'User' });
  
  // NEW: Store the raw transaction data to pass to the receipt page
  const [rawTxData, setRawTxData] = useState(null);
  ...
  useEffect(() => {
    const fetchSpecificTransaction = async () => {
      ...
      try {
        // 1. Fetch profile for the header
        ...
        if (txData) {
          setSummaryData(txData);
          setRawTxData(txData); // Store for passing to receipt
        }
      } catch (err) {
  ...
  return (
    <div className="flex min-h-screen bg-[#F8FAFC] font-sans text-slate-700 relative overflow-x-hidden">
      ...
      <main className="flex-1 lg:ml-64 min-h-screen flex flex-col w-full">
        ...
        <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-4 md:px-8 sticky top-0 z-20">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2 text-sm text-slate-400 font-medium">
              ...
              <span className="text-[#1e40af] font-bold bg-blue-50 px-3 py-1.5 rounded-lg">Success</span>
            </div>
          </div>
        ...
        <div className="flex-1 flex items-center justify-center p-4 md:p-6 pb-20">
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-xl bg-white rounded-[32px] shadow-2xl shadow-blue-900/5 border border-slate-100 overflow-hidden relative min-h-[400px] flex flex-col">
            
            {isLoading ? (
              <div className="flex-1 flex flex-col items-center justify-center p-12">
                <Loader2 className="animate-spin text-blue-500 mb-4" size={32} />
                <p className="text-slate-500 font-medium text-sm">Securing transaction to ledger...</p>
              </div>
            ) : (
              /* Success Content */
              <div className="p-8 md:p-12 flex flex-col items-center text-center">
                <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.1, type: "spring", stiffness: 200 }} className="w-16 h-16 bg-emerald-50 rounded-full flex items-center justify-center text-emerald-500 mb-6 shadow-sm ring-4 ring-emerald-50/50">
                ...
                {/* --- THE BRIDGE BOX --- */}
                <div className={`w-full bg-slate-50 rounded-2xl p-6 border mb-8 relative group transition-colors ${copied ? 'border-emerald-200 bg-emerald-50/30' : 'border-slate-200'}`}>
                  <div className="flex items-center justify-between mb-3">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-1.5">
                      <LinkIcon size={12} /> Share Confirmation Link
                    </p>
                    <span className="text-[9px] font-bold text-[#1e40af] bg-blue-50 px-2 py-0.5 rounded-full border border-blue-100 uppercase tracking-wider">
                      Action Required
                    </span>
                  </div>
                  
                  <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
                    <div className="flex-1 bg-white border border-slate-200 rounded-xl px-4 py-3 text-xs font-mono text-slate-500 truncate select-all shadow-sm text-left">
                      {verificationLink}
                    </div>
                    <button 
                      onClick={handleCopy}
                      className={`px-5 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all shadow-md flex items-center justify-center gap-2 shrink-0 ${
                        copied 
                          ? 'bg-emerald-500 text-white shadow-emerald-200 scale-[0.98]' 
                          : 'bg-[#1e40af] text-white hover:bg-blue-800 hover:-translate-y-0.5'
                      }`}
                    >
                  ...
                  <p className="text-[10px] text-slate-500 mt-4 text-left leading-relaxed">
                    <span className="font-bold text-slate-700">* Important:</span> Send this link to <span className="font-bold text-slate-900">{summary.clientName}</span>. Once they verify, your Trust Score will increase.
                  </p>
                </div>
                ...
                <div className="w-full border-t border-slate-100 pt-6 mb-8">
                  ...
                  <div className="flex justify-between items-center">
                    <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Ref ID</span>
                    <span className="text-xs font-mono font-bold text-slate-600 bg-slate-50 px-2 py-1 rounded-md">{summary.refId}</span>
                  </div>
                </div>

                {/* Actions */}
                <div className="flex flex-col-reverse sm:flex-row w-full gap-3">
                  <button onClick={() => navigate('/dashboard')} className="flex-1 py-3.5 border border-slate-200 bg-white text-slate-600 rounded-xl font-bold text-xs hover:bg-slate-50 transition-all uppercase tracking-wide shadow-sm">
                    Dashboard
                  </button>
                  
                  {/* POLISHED: Now passes data to receipt securely */}
                  <button 
                    onClick={() => rawTxData ? navigate('/transaction-receipt', { state: { transactionData: rawTxData } }) : navigate('/transaction-dashboard')} 
                    className="flex-1 py-3.5 bg-[#0f172a] text-white rounded-xl font-bold text-xs shadow-lg hover:bg-slate-800 transition-all uppercase tracking-wide flex items-center justify-center gap-2"
                  >
